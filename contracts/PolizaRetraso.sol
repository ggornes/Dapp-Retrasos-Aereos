...
//*******************************************************************************************/
// 
  function getPolizasTodas() public constant returns(uint[]) {
    if(ContadorPol == 0) {
      return new uint[](0);
    }
    ...
    for (uint i = 1; i <= ContadorPol; i++) {
      //if (pols[i].asegurador == 0x0) { //solicitada y no emitida
...
/****************************************************************************************** */
  ...
  function getPolizasSolicitadas() public constant returns(uint[]) {
    if(ContadorPol == 0) {
      return new uint[](0);
    }
...
/*********************************************************************************/
  ...
  function EmitirPoliza(uint _id) payable public {
    ...
    require(pol.asegurador == 0x0); //que no tenga previamente un asegurador
...
//Con esta funcion simulamos el evento aleatorio y elegimos el multiplicador correspondiente al Tretraso
  function Set_tretraso(uint _tretrasoelegido) {
    contador++;
    ...
    if (pol.result != 5 && pol.asegurador == 0x0) { //Se solicitó póliza, sucedió el evento y no se emitió
